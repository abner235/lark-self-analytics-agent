...
# ---- 必填（安全相关）----
# 发起人白名单：空格分隔的 open_id（ou_xxx）。fail-closed：留空时退回「仅 OWNER_AT 本人可触发」；
#   OWNER_AT 也空则没有任何人能触发。群成员资格 ≠ 数据权限，务必显式列出可触发的人，
#   否则任何被拉进白名单群的人都能借 owner 凭证取数。
...
# ---- 护栏（有默认值，按需覆盖）----
export MAX_RETRY=1               # 分析失败后自动重试次数（0=不重试，默认 1）
...
# --allowedTools 值会被错误拆词。
#   - ⚠️ 别放开整个 Bash（白名单内用户可借 prompt 注入跑任意 shell）：把 Bash 收口到取数 skill
#     实际运行的命令前缀。先观察该 skill 实际执行的命令（看 SKILL.md / 跑一次看 tool 调用），再写前缀；
#     前缀要和实际命令首 token 完全一致（实测 ./mcq 要写 Bash(./mcq:*)，写 Bash(mcq:*) 不匹配）。
#   - ⚠️ 不要加 Read/Grep/Glob：WORKDIR 默认 $HOME，这三个工具接受任意绝对路径，注入即可读
#     ~/.lark-cli token / mc-query cookie / ~/.ssh / 本 config 并回贴进群（凭证级泄露）。表结构/口径
#     由取数 skill 的 SKILL.md 携带，运行时不需要文件读取工具。确需读本地 schema 时用 Read(./schema/**)。
#   - ⚠️⚠️ 授权裸 Bash(mc-query:*) 等于让发起人能跑任意 SQL，用 owner 全权凭证拉到 owner 可见的
#     任何表（含客户 PII/未脱敏交易）——这是越权取数的主口子。安全做法二选一：
#       (A) 确认生产 mc-query 对本账号在服务端强制了表/字段级权限（取不该取的表会被服务端拒）；或
#       (B) 仿照 sandbox/mcq-safe，写一个先过 sql-validator 校验表/字段白名单再执行的 mc-query-safe
#           包装器，把这里改成 Bash(mc-query-safe:*)，不要授权裸 mc-query。
#     在确认 (A) 或落地 (B) 之前，不要把本 bridge 接到真实生产数据。
CLAUDE_ARGS=(--allowedTools "Skill Bash(mc-query:*) Bash(datawind:*)" --max-turns 30)
