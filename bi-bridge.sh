...
audit() {
  ...
  local task_trunc="${6:0:200}"
  printf '{"ts":"%s","message_id":"%s","chat_id":"%s","sender_id":"%s","status":"%s","duration_s":%s,"bot":"%s","task":%s}\n' \
    "$(date '+%Y-%m-%dT%H:%M:%S%z')" "$1" "$2" "$3" "$4" "$5" "$BOT_NAME" \
    "$(printf '%s' "$task_trunc" | sed 's/\\/\\\\/g; s/"/\\"/g' | awk '{printf "\"%s\"",$0}')" \
    >> "$AUDIT_LOG"
  log_to_base "$@" || true
}
...
run_task() {
  ...
  local marker="$STATE_DIR/running/$mid"; : > "$marker"
...
handle_event() {
  local line="$1"
  # 解析投影后的字段（consume 已用 --jq 投成 {message_id,chat_id,sender_id,content}）
  local mid chat sender content
  mid=$(printf '%s' "$line"     | jq -r '.message_id // empty' 2>/dev/null)   || return 0
  ...
  # 4.1 去重（message_id 唯一）
  local seen="$STATE_DIR/seen/$mid"
  [[ -e "$seen" ]] && return 0
  : > "$seen"
...
JQ_PROJECT='select(.chat_type=="group" and .message_type=="text") | {message_id, chat_id, sender_id, content}'
